...
createMenuItem('burrito',5,'breakfast')
createMenuItem('beer',2,'drink')
createMenuItem('coke',1,'drink')
...
export const burger = {
  ...
  price: 18, 
  category: "Lunch", 
  discount: function(line){
    if (line=="teacher" || line=="student"){
      let correctedprice=0;
      correctedprice=this.price*0.75;
      return correctedprice;
    }
    else{
      let correctedprice=0;
      correctedprice=this.price*0.9;
      return correctedprice;
    }
  }
}
...
*/
reviews.push({name:'Michael',rating:5,feedback:'like this place!'})
console.log(reviews)
...
function getReviewByIndex(line,num) {
  return(`${line[num].name} gave the restaurant a ${line[num].rating} star review, and their feedback was: ${line[num].feedback}`)
}
...
function getLastReview(line) {
  let k=0;
  k=line.length;
  return(`${line[k-1].name} gave the restaurant a ${line[k-1].rating} star review, and their feedback was: ${line[k-1].feedback}`)
} 
